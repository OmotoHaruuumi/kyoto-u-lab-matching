...
// Detail modal
// ---------------------------------------------------------------------------
type AnyLab = LabResult | LabListItem;

function DetailModal({ lab, onClose }: { lab: AnyLab; onClose: () => void }) {
  // Close on Escape key
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <div className="p-8">
          ...
          {/* Match highlights — only for search results */}
          {"matched_chunks" in lab && lab.matched_chunks.length > 0 && (
            <div className="mb-6">
...
// ---------------------------------------------------------------------------
// Shared LabCard component
// ---------------------------------------------------------------------------
function LabCard({ lab, onSelect }: { lab: AnyLab; onSelect: (lab: AnyLab) => void }) {
  const isSearchResult = "matched_chunks" in lab;
  return (
    <div className="group relative bg-[#1a2333]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 hover:border-indigo-400/50 transition-all duration-300 hover:shadow-[0_0_30px_-5px_rgba(99,102,241,0.3)] hover:-translate-y-1">
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            {isSearchResult && (
              <span className="text-xs font-mono font-medium px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 shrink-0 ml-2">
                Score: {(lab as LabResult).total_score.toFixed(3)}
              </span>
            )}
          </div>
          {lab.name_en && <p className="text-sm text-slate-400 mb-2">{lab.name_en}</p>}
        ...
        {isSearchResult && (
          <>
            ...
            <div className="mt-2">
              <h3 className="text-xs uppercase tracking-wider text-slate-500 mb-3 font-semibold">マッチ理由</h3>
              <ul className="space-y-3">
                {(lab as LabResult).matched_chunks.slice(0, 2).map((chunk, idx) => (
...
function Pagination({ current, total, onChange }: { current: number; total: number; onChange: (p: number) => void }) {
  return (
    <div className="mt-10 flex items-center justify-center gap-4">
      ...
      <span className="text-slate-400 text-sm">{current} / {total} ページ</span>
      <button
...
export default function SearchPage() {
  ...
  const [selectedLab, setSelectedLab] = useState<AnyLab | null>(null);
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1c] text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <div className="relative z-10 container mx-auto px-4 py-16 flex flex-col items-center">
        {/* Header */}
        <div className={`transition-all duration-700 ease-in-out flex flex-col items-center w-full ${hasSearched && tab === "search" ? "mt-8" : "mt-16"}`}>
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-300 mb-4 text-center">
            Kyoto Univ. Lab Match
          </h1>
          <p className="text-slate-400 text-lg md:text-xl text-center max-w-2xl mb-8">
            研究課題や興味を自然言語で入力すると、関連する京都大学の研究室を探せます。
          </p>

          {/* Tab switcher */}
          <div className="flex gap-1 p-1 bg-slate-800/60 rounded-xl mb-8 border border-white/10">
            <button
              onClick={() => setTab("search")}
              className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${tab === "search" ? "bg-indigo-600 text-white shadow" : "text-slate-400 hover:text-white"}`}
            >
              検索
            </button>
            <button
              onClick={() => setTab("list")}
              className={`px-6 py-2 rounded-lg text-sm font-medium transition-all ${tab === "list" ? "bg-indigo-600 text-white shadow" : "text-slate-400 hover:text-white"}`}
            >
              一覧
            </button>
          </div>
            ...
            {!isLoading && results.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pagedResults.map((lab) => (
                  <LabCard key={lab.lab_id} lab={lab} onSelect={setSelectedLab} />
                ))}
              </div>
            )}
        ...
        {tab === "list" && (
          <div className="mt-10 w-full max-w-5xl">
            ...
            {!listLoading && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {pagedListLabs.map((lab) => (
                  <LabCard key={lab.lab_id} lab={lab} onSelect={setSelectedLab} />
                ))}
              </div>
            )}
            {!listLoading && filteredLabs.length > RESULTS_PER_PAGE && (
              <Pagination current={listPage} total={listTotalPages} onChange={setListPage} />
            )}
          </div>
        )}
      </div>
